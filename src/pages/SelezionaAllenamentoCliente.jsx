  ...
  useEffect(() => {
    async function fetchAll() {
      ...
      const { data: plans } = await supabase
        .from('workout_plans').select('id, title, client_id, is_active').eq('is_active', true)
    ...
    <>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'18px 22px'}}>
        ...
        {!loading && clients.map(c => (
          <div key={c.id} onClick={() => c.plan ? navigate(`/modifica-allenamento/${c.plan.id}`) : null}
            ...
            <div style={{flex:1}}>
              ...
              {c.plan
                ? <div style={{fontSize:12,color:'#3B6D11',marginTop:2}}>✓ {c.plan.title}</div>
                : <div style={{fontSize:12,color:'#888780',marginTop:2}}>Nessuna scheda attiva</div>
              }
            </div>
            {c.plan
              ? <i className="ti ti-chevron-right" style={{fontSize:18,color:'#D4570A'}}/>
              : <span style={{fontSize:11,color:'#888780',background:'#F5F3EF',padding:'3px 8px',borderRadius:8}}>Nessuna scheda</span>
            }
          </div>
        ))}
      </div>
