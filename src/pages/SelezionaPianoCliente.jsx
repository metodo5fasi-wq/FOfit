  ...
  useEffect(() => {
    async function fetch() {
      const { data: plans } = await supabase
        .from('meal_plans')
        .select('id, title, kcal_target, client_id, profiles(full_name)')
        .eq('is_active', true)
      ...
      <div style={{flex:1, overflowY:'auto', padding:'18px 22px'}}>
        ...
        {!loading && clientsWithPlan.length === 0 && (
          <div style={{textAlign:'center', padding:'40px 20px', background:'white', borderRadius:12, border:'0.5px solid #E0DDD6'}}>
            <i className="ti ti-clipboard-x" style={{fontSize:44, color:'#E0DDD6', display:'block', marginBottom:14}}/>
            <div style={{fontSize:14, fontWeight:600, color:'#111', marginBottom:6}}>Nessun piano attivo</div>
            <div style={{fontSize:13, color:'#888780'}}>Importa prima un piano alimentare per i tuoi clienti.</div>
          </div>
        )}

        {!loading && clientsWithPlan.map(plan => (
          <div key={plan.id} onClick={() => navigate(`/modifica-piano/${plan.id}`)}
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:12, color:'#888780', marginTop:2}}>{plan.title} · {plan.kcal_target} kcal</div>
            </div>
