  ...
  async function savePlan() {
    ...
    try {
      const { data: planData, error: planErr } = await supabase.from('meal_plans').insert({
        ...
        kcal_target: Math.round(parseFloat(targets.kcal_target)) || 2000,
        protein_target_g: Math.round(parseFloat(targets.protein_target_g)) || 150,
        carbs_target_g: Math.round(parseFloat(targets.carbs_target_g)) || 200,
        fat_target_g: Math.round(parseFloat(targets.fat_target_g)) || 65,
        notes: planNotes,
        is_active: true,
      }).select().single()
