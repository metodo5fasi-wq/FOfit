...
Panca piana | Petto | https://youtube.com/watch?v=... | Scendi controllato fino al petto | 4x8-10 | 90sec
Croci manubri | Petto | | Movimento ad arco | 3x12 | 60sec

Giorno B - Schiena e Bicipiti
Trazioni | Schiena | https://youtube.com/watch?v=... | Presa prona, scapole addotte | 4x6-8 | 90sec
Curl bilanciere | Bicipiti | | Gomiti fermi | 3x10 | 60sec`
...
export default function ImportaAllenamento() {
  ...
  function reset() {
    setStep(1); setRawText(''); setParsedWorkout(null)
    setSelectedClient(''); setPlanTitle('Scheda allenamento'); setError('')
  }
    ...
    <>
      ...
      <div style={s.page}>
        ...
        {step === 1 && (
          <div style={s.card}>
            ...
            <div style={{fontSize:12,color:'#888780',lineHeight:1.6,marginBottom:14}}>
              Scrivi per ogni esercizio: <strong>nome | gruppo muscolare | link video | descrizione | serie x reps | recupero</strong>, raggruppati per giorno.
            </div>
            <textarea style={{...s.textarea, height:260}} placeholder={EXAMPLE} value={rawText} onChange={e=>setRawText(e.target.value)}/>
            {error && <div style={{fontSize:12,color:'#E24B4A',margin:'10px 0'}}>{error}</div>}
            <button style={{...s.btn,marginTop:12}} onClick={elabora} disabled={loading}>
        ...
        {step === 2 && parsedWorkout && (
          <>
            <div style={{...s.card, background:'#EAF3DE', border:'0.5px solid #3B6D11'}}>
              <div style={{fontSize:13,fontWeight:600,color:'#3B6D11',marginBottom:4}}>
                ✓ Scheda elaborata — {parsedWorkout.days.length} giorni, {totalExercises} esercizi
              </div>
              <div style={s.grid2}>
                <div>
              ...
              <div style={{fontSize:11,color:'#3B6D11',marginTop:8}}>
                ...
                Salvando, questa scheda diventerà l'unica attiva per il cliente (le precedenti verranno disattivate).
              </div>
            </div>
