...
async function makeVapidAuth(audience) {
  const header = b64url(JSON.stringify({ typ: 'JWT', alg: 'ES256' }))
  const now = Math.floor(Date.now() / 1000)
  const payload = b64url(JSON.stringify({ aud: audience, exp: now + 43200, sub: VAPID_EMAIL }))
  ...
  return `vapid t=${unsigned}.${b64url(Buffer.from(sig))},k=${VAPID_PUBLIC}`
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    const authorization = await makeVapidAuth(audience)
