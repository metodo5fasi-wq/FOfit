...
export default async function handler(req, res) {
  ...
  try {
    // ── MODALITÀ GENERA DA ZERO ──────────────────────────────
    if (req.body.mode === 'generate') {
      const { kcal, protein, carbs, fat, meals_per_day, foods_liked, foods_avoided, lifestyle, goal, diet_type } = req.body.preferences
      ...
      const userPrompt = `Crea un piano alimentare personalizzato DIVERSO ogni giorno per 7 giorni.

PARAMETRI:
- Kcal: ${kcal} | Proteine: ${protein}g | Carbo: ${carbs}g | Grassi: ${fat}g
- Pasti al giorno: ${meals_per_day}
- Obiettivo: ${goal}
- Tipo dieta: ${diet_type || 'lineare'}
- Alimenti preferiti: ${foods_liked || 'vari'}
- Da evitare: ${foods_avoided || 'nessuno'}
- Stile vita: ${lifestyle || 'standard'}

REGOLE:
- 7 giorni DIVERSI, non ripetere gli stessi pasti
- Per ogni alimento 1-2 opzioni alternative
- Macro bilanciati ogni giorno (±5%)
- Usa gli alimenti preferiti come base, varia le preparazioni
- Nomi pasti italiani: colazione, spuntino, pranzo, pre-workout, cena, merenda
- Kcal e macro precisi per ogni alimento

JSON da restituire:
{"varianti":[{"nome":"Lunedì","kcal":2000,"proteine_g":150,"carboidrati_g":200,"grassi_g":65,"pasti":[{"nome":"Colazione","tipo":"colazione","orario":"07:30","alimenti":[{"nome":"Fiocchi avena","quantita_g":80,"kcal":300,"proteine_g":10,"carboidrati_g":55,"grassi_g":5,"opzioni":[{"nome":"Alternativa","quantita_g":80,"kcal":290,"proteine_g":9,"carboidrati_g":52,"grassi_g":4}]}]}]}]}`

      const aiText = await callAI(apiKey, systemPrompt, userPrompt, 8000)
