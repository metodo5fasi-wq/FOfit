...
{"varianti":[{"nome":"Lunedì","kcal":${Math.round(kcal||2000)},"proteine_g":${Math.round(protein||150)},"carboidrati_g":${Math.round(carbs||200)},"grassi_g":${Math.round(fat||65)},"pasti":[{"nome":"Colazione","tipo":"colazione","orario":"07:30","alimenti":[{"nome":"Fiocchi avena","quantita_g":80,"kcal":300,"proteine_g":10,"carboidrati_g":55,"grassi_g":5,"opzioni":[{"nome":"Yogurt greco","quantita_g":200,"kcal":120,"proteine_g":20,"carboidrati_g":6,"grassi_g":1}]}]}]}]}`
    ...
    if (req.body.mode === 'parse_day') {
      ...
      const prompt = `Sei un parser di piani alimentari. Estrai i pasti dal testo e restituisci SOLO JSON valido.

STRUTTURA: {"pasti":[{"nome":"Colazione","tipo":"colazione","orario":"07:30","alimenti":[{"nome":"Latte","marca":"","quantita_g":300,"kcal":150,"proteine_g":10,"carboidrati_g":14,"grassi_g":6,"opzioni":[]}],"alternative":[]}]}

TIPI: colazione, spuntino, pranzo, pre-workout, post-workout, cena, merenda, altro
DOPO PALESTRA=post-workout | DOPO LAVORO=spuntino | PRIMA DI DORMIRE=merenda | DOPO PRANZO=spuntino

Usa i valori kcal e macro gia presenti nel testo. Ignora righe NB: e note tra parentesi.

TESTO:
${dayText.substring(0, 3000)}`

      const aiText = await callAI(apiKey, SYSTEM, prompt, 2000)
      const parsed = extractJSON(aiText)
      if (!parsed?.pasti?.length) {
        const prompt2 = 'Estrai pasti da questo testo come JSON {"pasti":[{"nome":"Colazione","tipo":"colazione","orario":"","alimenti":[{"nome":"alimento","marca":"","quantita_g":100,"kcal":100,"proteine_g":10,"carboidrati_g":10,"grassi_g":5,"opzioni":[]}],"alternative":[]}]}. Testo: ' + dayText.substring(0, 2000)
        const aiText2 = await callAI(apiKey, SYSTEM, prompt2, 2000)
        const parsed2 = extractJSON(aiText2)
        return res.status(200).json({ pasti: parsed2?.pasti || [], macros: macros || null })
      }
